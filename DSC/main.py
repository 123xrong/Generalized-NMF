...
from post_clustering import spectral_clustering, acc, nmi, nmf_coeff
...
def train(model,  # type: DSCNet
    ...
    for epoch in range(epochs):
        ...
        if epoch % show == 0 or epoch == epochs - 1:
            ...
            print('Epoch %02d: loss=%.4f, acc=%.4f, ari=%.4f, nmi=%.4f' %
                  (epoch, loss.item() / y_pred.shape[0], acc(y, y_pred), ari(y, y_pred), nmi(y, y_pred)))
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        C_final = dscnet.self_expression.Coefficient.detach().cpu().numpy()
        

        # Apply NMF on the sparse self-expressive matrix
        print("Running NMF on sparse coefficient matrix C...")
        acc_nmf, ari_nmf, nmi_nmf, pred_labels_nmf, W_nmf, H_nmf = nmf_coeff(
            C_final,
        ...
        print("NMF Results after DSC:")
        print("ACC = {:.4f}".format(acc_nmf))
        print("ARI = {:.4f}".format(ari_nmf))
        print("NMI = {:.4f}".format(nmi_nmf))
