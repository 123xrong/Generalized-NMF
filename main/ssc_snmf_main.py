...
def arg_parser():
    ...
    parser.add_argument('--alpha', type=float, default=1, help='Regularization parameter for ssc')
    parser.add_argument('--lmbda', type=float, default=0.05, help='Regularization parameter for sparse NMF')
    return parser.parse_args()

def main(m, r, n_k, K, sigma=0.0, alpha=1, lmbda=0.05, random_state=None, max_iter=100):

    wandb.init(
        project="coneClustering",
        name = "SSC-SNMF-synthetic"
    )
    # 1. Generate distinct subspace data
    X, true_labels = data_simulation(m, r, n_k, K, sigma=sigma, random_state=random_state)

    acc, ARI, NMI, reconstruction_error = ssc_sparse_nmf_baseline(X, r, K, true_labels=true_labels, alpha=alpha, l1_reg=lmbda)
...
if __name__ == "__main__":
    ...
    main(m, r, n_k, K, sigma, alpha, lmbda, random_state, max_iter)